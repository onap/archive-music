...
public class FeedReturnStreamingOutput implements StreamingOutput {
    ...
    public FeedReturnStreamingOutput() {
        this.jsonInsertObj = new JsonSelect();
        jsonInsertObj.setKeyspaceName("tomtest");
        jsonInsertObj.setTableName("usaccidents");
        rowParams = new MultivaluedHashMap<String, String>();
    }
    ...
    public void write(OutputStream output) throws IOException, WebApplicationException {
        ...
        MusicDataStore ds = getMusicDataStore();

        try {
            queryObject = jsonInsertObj.genSelectQuery(rowParams);
            
            SimpleStatement statement = getSimpleStatement(queryObject);
    ...
    public MusicDataStore getMusicDataStore() {
        return new MusicDataStore();
    }
    
    /* For Junit testing purpose only */
    public SimpleStatement getSimpleStatement(PreparedQueryObject queryObject) {
        return new SimpleStatement(queryObject.getQuery(), queryObject.getValues().toArray());
    }
